...
@Stateless
@LocalBean
public class SalesOrderDetailForInventoryBean extends SuperBean<SalesOrderDetailForQuery> {

    public SalesOrderDetailForInventoryBean() {
    ...
    public int getRowCount(Map<String, Object> filters) {
        ...
        sb.append("SELECT COUNT(e) FROM SalesOrderDetailForQuery e WHERE ((e.issqty - e.inqty) > 0)  ");
    ...
    public List<SalesOrderDetailForQuery> findByFilters(Map<String, Object> filters, int first, int pageSize, Map<String, String> orderBy) {
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT e FROM SalesOrderDetailForQuery e WHERE ((e.issqty - e.inqty) > 0)  ");
        sb.append(" AND (e.salesOrder.status<>'N') AND (e.status<>'MC') ");
        if (filters != null) {
            this.setQueryFilter(sb, filters);
        }
