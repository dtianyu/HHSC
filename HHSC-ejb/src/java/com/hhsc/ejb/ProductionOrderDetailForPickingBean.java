...
@Stateless
@LocalBean
public class ProductionOrderDetailForPickingBean extends SuperBean<ProductionOrderDetailForQuery> {

    public ProductionOrderDetailForPickingBean() {
    ...
    public int getRowCount(Map<String, Object> filters) {
        ...
        sb.append("SELECT COUNT(e) FROM ProductionOrderDetailForQuery e WHERE ((e.qty - e.issqty) > 0)  ");
        sb.append(" AND (e.productionOrder.salesstatus='V') ");
        if (filters != null) {
    ...
    public List<ProductionOrderDetailForQuery> findByFilters(Map<String, Object> filters, int first, int pageSize, Map<String, String> orderBy) {
        StringBuilder sb = new StringBuilder();
        sb.append("SELECT e FROM ProductionOrderDetailForQuery e WHERE ((e.qty - e.issqty) > 0)  ");
        sb.append(" AND (e.productionOrder.salesstatus='V') ");
        if (filters != null) {
            this.setQueryFilter(sb, filters);
        }
